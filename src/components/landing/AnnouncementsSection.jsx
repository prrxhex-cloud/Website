...
function AnnouncementCard({ item, isAdmin, onEdit, onDelete }) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="clean-card p-6 bg-[var(--bg-card)] border border-[var(--border-color)] relative group shadow-md text-left"
    >
      {item.pinned && (
        <div className="absolute top-4 right-4 flex items-center gap-1 text-xs font-bold text-amber-400 bg-amber-500/15 px-2.5 py-1 rounded-full border border-amber-500/30">
          <Pin className="w-3.5 h-3.5" /> PINNED
        </div>
      )}
      <div className="flex items-start gap-4">
        <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 bg-[var(--bg-subtle)] border border-[var(--border-color)]" style={{ color: cfg.color }}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="font-inter text-sm text-[var(--text-primary)] leading-relaxed">{item.content}</p>
          {item.author_name && (
      ...
      {isAdmin && (
        <div className="flex gap-2 pt-4 border-t border-[var(--border-color)] mt-4 justify-end">
          <button onClick={() => onEdit(item)} className="p-2 rounded-lg bg-[var(--bg-subtle)] border border-[var(--border-color)] hover:border-[#06b6d4] text-[var(--text-primary)] transition-colors">
            <Edit2 className="w-4 h-4" />
          </button>
          <button onClick={() => onDelete(item.id)} className="p-2 rounded-lg bg-rose-500/15 border border-rose-500/30 text-rose-400 hover:bg-rose-500/25 transition-colors">
...
export default function AnnouncementsSection({ isAdmin }) {
  ...
  return (
    <section id="announcements" className="py-16 px-4 bg-[var(--bg-main)] border-b border-[var(--border-color)] text-[var(--text-primary)] transition-colors duration-300">
      <div className="max-w-[1240px] mx-auto">
        ...
        {items.length === 0 ? (
          <div className="text-center py-16 clean-card bg-[var(--bg-card)] border border-[var(--border-color)] rounded-3xl shadow-sm">
            <Newspaper className="w-10 h-10 text-[var(--text-muted)] mx-auto mb-3" />
            <p className="font-outfit text-[var(--text-heading)] font-bold text-base">No announcements posted yet.</p>
          </div>
        ) : (
