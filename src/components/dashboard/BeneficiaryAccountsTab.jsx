...
// import { clearBeneficiaryCache } from '@/utils/beneficiaryVerifier'; // Ignore cache for now
...
export default function BeneficiaryAccountsTab() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, 'beneficiary_accounts'), orderBy('created_date', 'desc'), limit(100));
  ...
  const save = async () => {
    ...
    try {
      if (form.id) {
        await updateDoc(doc(db, 'beneficiary_accounts', form.id), {
  ...
  const toggleActive = async (acc) => {
    try {
      await updateDoc(doc(db, 'beneficiary_accounts', acc.id), { active: !acc.active });
